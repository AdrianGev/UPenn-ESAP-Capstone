...
def main():
    ...
    for i in range(100):
        ...
        while True:
            ...
            game.draw_figures()
            if game.game_over:
                if game.show_game_over_message() == "tie":
                    tie += 1
                elif game.show_game_over_message() == "random":
                    rand_win += 1
                else:
                    opt_win += 1
                break
            pygame.display.update()
        list_of_turns.append(total_turns)
            
    
    print("optimized won", opt_win)
    print("random win", rand_win)
    print("draw", tie)

    # Labels and values
    labels = ['Random bot wins', 'Optimal bot wins', 'ties']
    values = [rand_win, opt_win,tie]
    ...
    plt.hist(list_of_turns, bins=15, edgecolor='black')

    # Label axes
    plt.xlabel('total move count')
    plt.ylabel('Frequency')
    plt.title('total moves count histogram')
    plt.show()
